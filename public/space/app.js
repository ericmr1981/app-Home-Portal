...
function calculateOrbitDuration(orbitRadius) {
  // Base duration for Earth (orbitRadius = 60 in our adjusted scale)
  ...
  // Calculate duration based on orbital radius using Kepler's law
  const duration = earthDuration * Math.pow(orbitRadius / earthRadius, keplerExponent);

  // Clamp to reasonable bounds (min 3s for Mercury, max 120s for Neptune)
  // Extended max to accommodate larger orbits
  return Math.max(3, Math.min(120, duration));
}
...
function createPlanet(planet, index) {
  ...
  // Position the planet container at the right spot on the orbit
  planetContainer.style.left = `${planet.orbitRadius}px`;  // Position on the right side of the orbit
  ...
  // Position planet on the orbit edge (within the container)
  planetEl.style.left = '0px';
